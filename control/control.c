...
{
	ssize_t msg_size =
	    protobuf_send_message(sock, (ProtobufCMessage *) msg);
	if (msg_size < 0) {
...
{
	DaemonToController *resp =
	    (DaemonToController *) protobuf_recv_message(sock,
...
int main(int argc, char *argv[])
{
	...
	if (!strcasecmp(command, "list")) {
		msg.command =
		    CONTROLLER_TO_DAEMON__COMMAND__GET_CONTAINER_STATUS;
		has_response = true;
		goto send_message;
	}
	...
	if (!strcasecmp(command, "push_guestos_config")) {
		...
		INFO("Pushing cfg %s (len %zu), sig %s (len %zu), and cert %s (len %zu).", cfgfile, (size_t) cfglen, sigfile, (size_t) siglen, certfile, (size_t) certlen);

		msg.command =
		    CONTROLLER_TO_DAEMON__COMMAND__PUSH_GUESTOS_CONFIG;
	...
	if (!strcasecmp(command, "start")) {
		...
		for (int c, option_index = 0;
		     -1 != (c =
			    getopt_long(start_argc, start_argv, "k::s",
					start_options, &option_index));) {
			switch (c) {
			case 'k':
				container_start_params.key =
				    optarg ? optarg : DEFAULT_KEY;
				break;
			case 's':
		...
		msg.command =
		    CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_ALLOWAUDIO;
	} else if (!strcasecmp(command, "deny_audio")) {
		msg.command =
		    CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_DENYAUDIO;
		...
		msg.command =
		    CONTROLLER_TO_DAEMON__COMMAND__GET_CONTAINER_CONFIG;
		has_response = true;
	} else if (!strcasecmp(command, "ifaces")) {
		msg.command =
		    CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_LIST_IFACES;
		has_response = true;
	} else if (!strcasecmp(command, "assign_iface")
		   || !strcasecmp(command, "unassign_iface")) {
		AssignInterfaceParams assign_iface_params =
		    ASSIGN_INTERFACE_PARAMS__INIT;
		...
		for (int c, option_index = 0;
		     -1 != (c =
			    getopt_long(start_argc, start_argv, "i::p",
					assign_iface_options,
					&option_index));) {
			switch (c) {
			case 'i':
		...
		if (!strcasecmp(command, "assign_iface")) {
			msg.command =
			    CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_ASSIGNIFACE;
		} else if (!strcasecmp(command, "unassign_iface")) {
			msg.command =
			    CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_UNASSIGNIFACE;
		} else
...
send_message:
	...
	if (!strcasecmp(command, "run")) {
			...
			if (pid == -1) {
				...
				while (1) {
					...
					if (resp->code ==
					    DAEMON_TO_CONTROLLER__CODE__EXEC_OUTPUT)
					{
	...
	if (has_response) {
		...
		// TODO for now just dump the response in text format
			protobuf_dump_message(STDOUT_FILENO,
