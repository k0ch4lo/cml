...
int
ssl_verify_signature_from_digest(const char *cert_buf, size_t cert_len, const uint8_t *sig_buf, size_t sig_len,
				 const uint8_t *hash, size_t hash_len, const char *digest_algo)
{
	ASSERT(cert_buf);
	ASSERT(sig_buf);
	ASSERT(hash);
...
ssl_verify_signature_from_buf(uint8_t *cert_buf, size_t cert_len, const uint8_t *sig_buf,
			      size_t sig_len, const uint8_t *buf, size_t buf_len, const char *digest_algo)
...
//	DEBUG("Certificate uses hash algorithm %s", digest_algo);
	...
	if (0 > (ret = ssl_verify_signature_from_digest((char *)cert_buf, cert_len, sig_buf, sig_len, hash,
		     ...
		     const char *digest_algo)
{
	...
	int ret = ssl_verify_signature_from_digest(cert_buf, cert_len, sig_buf, sig_len, hash, hash_len, digest_algo);
