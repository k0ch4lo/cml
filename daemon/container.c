...
static time_t container_get_creation_time_from_file(container_t * container)
{
	...
	if (!file_exists(file_name_created)) {
		ret = time(NULL);
		if (file_write(file_name_created, (char *)&ret, sizeof(ret)) <
		    0) {
...
container_t *container_new_internal(const uuid_t * uuid,
				    const char *name,
				    container_type_t type,
				    bool ns_usr,
				    bool ns_net,
				    ...
				    char **assigned_devices)
{
	...
	if (!container->cgroups) {
		WARN("Could not initialize cgroups subsystem for container %s (UUID: %s)", container->name, uuid_string(container->uuid));
		goto error;
	}

	list_t *nw_name_list = hardware_get_nw_name_list();
	for (list_t * elem = nw_name_list; elem != NULL; elem = elem->next) {
		DEBUG("List element in nw_names_list: %s",
		      (char *)(elem->data));
	}
	...
	if (privileged && ns_net) {
		...
		for (list_t * elem = nw_mv_name_list; elem != NULL;
		     elem = elem->next) {
			DEBUG("List element in nw_names_list: %s",
			      (char *)(elem->data));
		}
	...
	for (list_t * elem = net_ifaces; elem != NULL; elem = elem->next) {
		...
		nw_mv_name_list =
		    list_append(nw_mv_name_list, mem_strdup(elem->data));
	}

	container->net =
	    c_net_new(container, ns_net, nw_name_list, nw_mv_name_list,
		      adb_port);
	if (!container->net) {
		WARN("Could not initialize net subsystem for container %s (UUID: %s)", container->name, uuid_string(container->uuid));
		goto error;
	}
	...
	if (!container->vol) {
		WARN("Could not initialize volume subsystem for container %s (UUID: %s)", container->name, uuid_string(container->uuid));
		goto error;
	}

	container->service = c_service_new(container);
	if (!container->service) {
		WARN("Could not initialize service subsystem for container %s (UUID: %s)", container->name, uuid_string(container->uuid));
		goto error;
	}
	...
	for (list_t * elem = container->feature_enabled_list; elem != NULL;
	     elem = elem->next) {
		DEBUG("Feature %s enabeld for %s", (char *)(elem->data),
		      container->name);
	}
	...
	container->time_created =
	    container_get_creation_time_from_file(container);
 ...
 */
/* TODO Error handling */
container_t *container_new(const char *store_path, const uuid_t * existing_uuid,
			   const char *config, size_t config_len)
{
	...
	config_filename =
	    mem_printf("%s/%s.conf", store_path, uuid_string(uuid));
	...
	container_config_t *conf =
	    container_config_new(config_filename, config, config_len);
	...
	if (!os) {
		WARN("Could not get GuestOS %s instance for container %s",
		     os_name, name);
		mem_free(config_filename);
		mem_free(images_dir);
		uuid_free(uuid);
	...
	if (current_guestos_version < new_guestos_version) {
		INFO("Updating guestos version from %" PRIu64 " to %" PRIu64
		     " for container %s", current_guestos_version,
		...
		INFO("Keeping current guestos version %" PRIu64
		     " for container %s", current_guestos_version, name);
	} else {
		WARN("The version of the found guestos (%" PRIu64
		     ") for container %s is to low", new_guestos_version, name);
		WARN("Current version is %" PRIu64 "; Aborting...",
	...
	const char *dns_server =
	    ...
	    container_config_get_dns_server(conf) : cmld_get_device_host_dns();
	...
	container_t *c =
	    container_new_internal(uuid, name, type, ns_usr, ns_net, priv, os,
				   config_filename,
				   images_dir, mnt, ram_limit, color, adb_port,
				   allow_autostart, feature_enabled,
				   dns_server, net_ifaces, allowed_devices,
				   assigned_devices);
	if (c)
...
void container_oom_protect_service(const container_t * container)
{
	...
	DEBUG
	    ("Setting oom_adj of trustme service (PID %d) in container %s to -17",
...
int container_write_config(container_t * container)
{
	...
	if (container->config_filename) {
		...
		WARN("Trying to write configless container to file... Try to set config" "filename first if you want to make the container config persistent");
		return -1;
	}
}
...
{
	return c_service_send_message(container->service,
...
static void container_cleanup(container_t * container)
{
	...
	if (container->stop_timer) {
		DEBUG("Remove container stop timer for %s",
	...
	if (container->start_timer) {
		DEBUG("Remove container start timer for %s",
		      container_get_description(container));
...
void container_sigchld_cb(UNUSED int signum, event_signal_t * sig, void *data)
{
	...
	DEBUG("SIGCHLD handler called for container %s with PID %d, exec pid: %d",
	      container_get_description(container), container->pid, container->active_exec_pid);
	...
	while ((pid = waitpid(-1, &status, WNOHANG))) {
		TRACE("Got exited child with PID: %d, exec PID: %d, exec pgid", pid, container->active_exec_pid);
		if (pid == container_pid) {
			TRACE("Entered first branch");
			if (WIFEXITED(status)) {
				INFO("Container %s terminated (init process exited with status=%d)", container_get_description(container), WEXITSTATUS(status));
			...
			if (errno == ECHILD)
				DEBUG
				    ("Process group of container %s terminated completely",
				     container_get_description(container));
			else
				WARN_ERRNO("waitpid failed for container %s",
					   container_get_description
					   (container));
			break;
		} else if (pid == container->active_exec_pid) {
			TRACE("Exec process exited. Closing sockets");
			shutdown(container->console_sock_container, SHUT_WR);
			shutdown(container->console_sock_cmld, SHUT_WR);
			container->active_exec_pid = -1;
		} else {
			DEBUG("Reaped a child with PID %d for container %s",
			      pid, container_get_description(container));
		}
	}

	TRACE("No more exited childs. Leaving SIGCHLD callback");
}

static int
container_close_all_fds_cb(UNUSED const char *path, const char *file,
			   UNUSED void *data)
{
	int fd = atoi(file);
...
static int container_start_child(void *data)
{
	...
	char *kvm_root =
	    mem_printf("/tmp/%s", uuid_string(container->uuid));
	...
	if (setpgid(0, 0) < 0) {
		WARN("Could not move process group of container %s",
		     container->name);
		goto error;
	}
	...
	char *root =
	    (container->type == CONTAINER_TYPE_KVM) ? kvm_root : "/";
	if (chdir(root) < 0) {
		WARN_ERRNO("Could not chdir to \"%s\" in container %s",
			   root, uuid_string(container->uuid));
		goto error;
	}
	...
	if (unshare(CLONE_NEWCGROUP) == -1)
		WARN_ERRNO
		    ("Could not unshare cgroup namespace, not supported by kernel.");
	else
	...
	if (!container->privileged) {
		DEBUG
		    ("Dropping all trustme-lsm privileges for container %s",
		     container_get_description(container));
		if (open
		    ("/sys/kernel/security/trustme/drop_privileges",
		     0) < 0)
			WARN_ERRNO
			    ("Could not drop trustme-lsm privileges");
	}
	...
	if (container->type == CONTAINER_TYPE_KVM) {
		...
		if (pid == 0) {	// child
			...
			char buffer[128];
			ssize_t read_bytes;
			char *kvm_log = mem_printf("%s.kvm.log",
						   container_get_images_dir
						   (container));
			read_bytes = read(fd_master, buffer, 128);
			file_write(kvm_log, buffer, read_bytes);
			while ((read_bytes =
				read(fd_master, buffer, 128))) {
				file_write_append(kvm_log, buffer,
						  read_bytes);
			}
	...
	if (container_get_state(container) != CONTAINER_STATE_SETUP) {
		DEBUG
		    ("After closing all file descriptors no further debugging info can be printed");
		if (container_close_all_fds()) {
			WARN("Closing all file descriptors failed, continuing anyway...");
		}
	}

	execve(guestos_get_init(container->os), container->init_argv,
	       container->init_env);
	WARN("Could not run exec for container %s",
	     uuid_string(container->uuid));
...
static void container_start_timeout_cb(event_timer_t * timer, void *data)
{
	...
	if (container_get_state(container) == CONTAINER_STATE_BOOTING) {
		WARN("Reached container start timeout for container %s and the container is still booting." " Killing it...", container_get_description(container));
...
static void
container_start_post_clone_cb(int fd, unsigned events, event_io_t * io,
			      void *data)
{
	char msg;
	container_t *container = data;
	...
	if (container_get_state(container) != CONTAINER_STATE_SETUP) {
		...
		container->start_timer =
		    event_timer_new(CONTAINER_START_TIMEOUT, 1,
...
int
container_run(container_t *container, char *cmd, char **argv)
{
	ASSERT(cmd);
...
int container_start(container_t * container)	//, const char *key)
{
	...
	if (container_get_state(container) != CONTAINER_STATE_STOPPED) {
		ERROR
		    ("Container %s is not stopped and can therefore not be started",
		     container_get_description(container));
		return CONTAINER_ERROR;
	}
	...
	void *container_stack_high =
	    (void *)((const char *)container_stack + CLONE_STACK_SIZE);
	...
	pid_t container_pid =
	    clone(container_start_child, container_stack_high, clone_flags,
		  container);
	if (container_pid < 0) {
	...
	/* close the childs end of the console task sockets */
	close(container->console_sock_container);
	 ...
	 * dies */
	event_signal_t *sig =
	    event_signal_new(SIGCHLD, container_sigchld_cb, container);
...
void container_kill(container_t * container)
{
	...
	DEBUG("Killing container %s with pid: %d",
	      container_get_description(container),
	      container_get_pid(container));

	if (kill(container_get_pid(container), SIGKILL)) {
		ERROR_ERRNO("Failed to kill container %s",
			    container_get_description(container));
	}
}
...
static void container_stop_timeout_cb(event_timer_t * timer, void *data)
{
	...
	container_t *container = data;
	DEBUG
	    ("Reached container stop timeout for container %s. Doing the kill now",
	     container_get_description(container));
...
int container_stop(container_t * container)
{
	...
	event_timer_t *container_stop_timer =
	    event_timer_new(CONTAINER_STOP_TIMEOUT, 1,
	...
	DEBUG
	    ("Stop container successfully emitted. Wait for child process to terminate (SICHLD)");
...
int
container_bind_socket_after_start(UNUSED container_t * container,
...
int container_freeze(container_t * container)
{
	...
	if (state == CONTAINER_STATE_FROZEN
	    || state == CONTAINER_STATE_FREEZING) {
...
container_wipe_image_cb(const char *path, const char *name, UNUSED void *data)
{
	...
	if (len >= 4 && !strcmp(name + len - 4, ".img")) {
		...
		DEBUG("Deleting image of container %s: %s",
		      container_get_description(container), image_path);
...
int container_wipe_finish(container_t * container)
{
	...
	if (dir_foreach
	    (container->images_dir, &container_wipe_image_cb, container) < 0) {
...
static void
container_wipe_cb(container_t * container, container_callback_t * cb,
		  UNUSED void *data)
{
	ASSERT(container);
...
int container_wipe(container_t * container)
{
	...
	if (container_get_state(container) != CONTAINER_STATE_STOPPED) {
		...
		if (!container_register_observer
		    (container, &container_wipe_cb, NULL)) {
...
void
container_unregister_observer(container_t * container,
			      container_callback_t * cb)
{
	ASSERT(container);
	ASSERT(cb);

	if (list_find(container->observer_list, cb)) {
		container->observer_list =
		    list_remove(container->observer_list, cb);
		mem_free(cb);
	}
...
void
container_set_connectivity(container_t * container,
			   container_connectivity_t connectivity)
{
	ASSERT(container);
...
void container_set_screen_on(container_t * container, bool screen_on)
{
	...
	if (screen_on) {
		DEBUG("Setting screen on for container %s",
		      container_get_description(container));
	} else {
		DEBUG("Setting screen off for container %s",
		      container_get_description(container));
	}
...
int
container_add_net_iface(container_t * container, const char *iface,
			bool persistent)
{
	ASSERT(container);
	...
	if (res || !persistent)
		return res;
	container_config_t *conf =
	    container_config_new(container->config_filename, NULL, 0);
...
int
container_remove_net_iface(container_t * container, const char *iface,
			   bool persistent)
{
	ASSERT(container);
	pid_t pid = container_get_pid(container);
