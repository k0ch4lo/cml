			     ...
			     bool send_last_line_info)
{
	...
	while (file_is_open) {
		if (read_low_level) {
			...
			bytes_read =
			    read(fp_low, line_low, LOGGER_ENTRY_MAX_LEN);
			if (bytes_read <= 0)
				break;
			char *first_string = line_low + HEADER_LENGTH;
			size_t string_len = strlen(first_string);
			char *second_string =
			    line_low + HEADER_LENGTH + string_len + 1;
			message.msg =
			    mem_printf("%s/%s", first_string, second_string);
		} else {
			size_t len = 0;
	...
	if (send_last_line_info) {
		...
		out.log_message = &message;
		if (protobuf_send_message(fd, (ProtobufCMessage *) & out) < 0) {
			ERROR("Could not sent last line info for %s",
			      log_file_name);
		}
	}
	mem_free(out.device_uuid);
 ...
 */
static ContainerStatus *control_container_status_new(const container_t *
						     container)
{
	...
	c_status->type =
	    control_container_type_to_proto(container_get_type(container));
	c_status->state =
	    control_container_state_to_proto(container_get_state(container));
...
//static void control_cb_confirm_exec(int fd, unsigned events, event_io_t * io,
//				    void *data)
//{
...
//
//	// wait for confirmation of successful exec
//	//TODO add timeout
//	ConsoleToDaemon *msg = (ConsoleToDaemon *)
//	    protobuf_recv_message
//	    (container_get_console_sock(container),
//	     &daemon_to_console__descriptor);
//
//
//	if (msg == NULL || msg->code == CONSOLE_TO_DAEMON__CODE__EXEC_FAILURE) {
//		DEBUG("Container exec response was empty or EXEC_FAILURE. Sending EXEC_END to client.");
//
//		DaemonToController out = DAEMON_TO_CONTROLLER__INIT;
//		out.code = DAEMON_TO_CONTROLLER__CODE__EXEC_END;
//		
//		if (protobuf_send_message
//		    (fd, (ProtobufCMessage *) & out) < 0) {
//			WARN("Could not send container PID to MDM");
//		}
//
//		DEBUG("Sent exec_end to client. Done serving request.");
//
//		return;
//	}
//
//
//				// occurs when executed process writes to stdout/stderr TODO check in container.c
//	DEBUG("Registering exec confirmation callback for cmld console socket");
//	event_io_t *event =
//	    event_io_new(container_get_console_sock(container),
//			 EVENT_IO_READ | EVENT_IO_WRITE | EVENT_IO_EXCEPT, control_cb_confirm_exec,
//			 control);
//	event_add_io(event);
//}
 ...
 */
static list_t *control_build_container_list_from_uuids(size_t n_uuids,
						       char **uuids)
{
	...
	if (n_uuids > 0) {	// uuid list given in incoming message
		for (size_t i = 0; i < n_uuids; i++) {
			container_t *container =
			    control_get_container_by_uuid_string(uuids[i]);
			if (container != NULL)
...
int control_send_message(control_message_t message, int fd)
{
	...
	case CONTROL_RESPONSE_CONTAINER_START_OK:
		out.response =
		    DAEMON_TO_CONTROLLER__RESPONSE__CONTAINER_START_OK;
		break;

	case CONTROL_RESPONSE_CONTAINER_START_LOCK_FAILED:
		out.response =
		    DAEMON_TO_CONTROLLER__RESPONSE__CONTAINER_START_LOCK_FAILED;
		break;

	case CONTROL_RESPONSE_CONTAINER_START_UNLOCK_FAILED:
		out.response =
		    DAEMON_TO_CONTROLLER__RESPONSE__CONTAINER_START_UNLOCK_FAILED;
		break;

	case CONTROL_RESPONSE_CONTAINER_START_PASSWD_WRONG:
		out.response =
		    DAEMON_TO_CONTROLLER__RESPONSE__CONTAINER_START_PASSWD_WRONG;
		break;

	case CONTROL_RESPONSE_CONTAINER_START_LOCKED_TILL_REBOOT:
		out.response =
		    DAEMON_TO_CONTROLLER__RESPONSE__CONTAINER_START_LOCKED_TILL_REBOOT;
		break;

	case CONTROL_RESPONSE_CONTAINER_START_EEXIST:
		out.response =
		    DAEMON_TO_CONTROLLER__RESPONSE__CONTAINER_START_EEXIST;
		break;
		       ...
		       control_t * control)
{
	...
	if (LOGF_PRIO_TRACE >= LOGF_LOG_MIN_PRIO) {
		char *msg_text =
		    protobuf_c_text_to_string((ProtobufCMessage *) msg, NULL);
	...
	case CONTROLLER_TO_DAEMON__COMMAND__LIST_GUESTOS_CONFIGS:{
			...
			for (size_t i = 0; i < n; i++) {
				guestos_t *os =
				    guestos_mgr_get_guestos_by_index(i);
			...
			DaemonToController out = DAEMON_TO_CONTROLLER__INIT;
			out.code =
			    DAEMON_TO_CONTROLLER__CODE__GUESTOS_CONFIGS_LIST;
	...
	case CONTROLLER_TO_DAEMON__COMMAND__GET_CONTAINER_STATUS:{
			// assemble list of relevant containers and allocate memory for result
			list_t *containers =
			    control_build_container_list_from_uuids
			    (msg->n_container_uuids,
			     msg->container_uuids);
			size_t n = list_length(containers);
			ContainerStatus **results =
			    mem_new(ContainerStatus *, n);

			// fill result with data from container
			for (size_t i = 0; i < n; i++) {
				container_t *container =
				    list_nth_data(containers, i);
				results[i] =
				    control_container_status_new(container);
			}
	...
	case CONTROLLER_TO_DAEMON__COMMAND__GET_CONTAINER_CONFIG:{
			...
			list_t *containers =
			    ...
			    (msg->n_container_uuids,
			     msg->container_uuids);
			size_t n = list_length(containers);
			ContainerConfig **results =
			    mem_new0(ContainerConfig *, n);
			...
			for (size_t i = 0; i < n; i++) {
				...
				TRACE
				    ("Getting container config for container %s",
				...
				if (!config_filename) {
					...
					TRACE
					    ("Container %s has config file; appending to list...",
					     container_get_name(container));
					...
					if (results[number_of_configs] == NULL) {
						WARN("The config file of container %s is missing. Skipping.", container_get_name(container));
						continue;
					}
					const char *uuid =
					    uuid_string(container_get_uuid
							(container));
					result_uuids[number_of_configs] =
					    mem_strdup(uuid);
					number_of_configs++;
				}
			}
			...
			if (number_of_configs == 0) {
				...
				TRACE("Got %zu container configs",
				      number_of_configs);
	...
	case CONTROLLER_TO_DAEMON__COMMAND__OBSERVE_STATUS_STOP:
		WARN("ControllerToDaemon command %d not implemented yet",
		     msg->command);
		break;
	...
	case CONTROLLER_TO_DAEMON__COMMAND__PUSH_GUESTOS_CONFIG:{
			if (!msg->has_guestos_config_file)
				...
				guestos_mgr_push_config
				    (msg->guestos_config_file.data,
				     msg->guestos_config_file.len,
	...
	case CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_ASSIGNIFACE:{
			...
			container_add_net_iface(container, net_iface,
						persistent);
		} break;
	...
	case CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_UNASSIGNIFACE:{
			char *net_iface = msg->assign_iface_params->iface_name;
			bool persistent = msg->assign_iface_params->persistent;
			container_remove_net_iface(container, net_iface,
						   persistent);
		} break;
	...
	case CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_EXEC_CMD:{
			if(container == NULL || container_get_active_exec_pid(container) != -1)
			{
				TRACE("Failed to exec. Wrong UUID or or already executing command in this container.");
			...
			else
			{
				...
				uint64_t i = 0;
				char ** exec_args = mem_alloc(sizeof(char *) * (msg->n_exec_args + 1));
				...
				container_run(container, msg->exec_command, exec_args);		
				//container_run(container, msg->exec_command, NULL);		
	
				/* connection succeeded so register socket for receiving data */
				//fd_make_non_blocking(container_get_console_sock
				//		     (container));
	...
	case CONTROLLER_TO_DAEMON__COMMAND__CONTAINER_EXEC_INPUT:{
			...
			{
				DEBUG("Failed to get active PID of exec'ed process. Doing nothing");
			}
		}
		break;

	default:
		WARN("Unknown ControllerToDaemon command: %d received",
		     msg->command);
		/* DO NOTHING */
	}
}
...
control_cb_recv_message(int fd, unsigned events, event_io_t * io, void *data)
{
	...
	if ((events & EVENT_IO_WRITE) && control->type == AF_INET) {
		...
		if (res != 0) {
			TRACE("res of getsockopt for %d says error %s", fd,
			      strerror(res));
			connection_error = true;
		} else {
			DEBUG("Connected to remote host %s:%d", control->hostip,
			      control->port);
			control->connected = true;
			container_t *container_a0 = cmld_containers_get_a0();
			char *imei = container_get_imei(container_a0);
			char *mac_address =
			    container_get_mac_address(container_a0);
			char *phone_number =
			    container_get_phone_number(container_a0);

			/* send LOGON_DEVICE message */
			DEBUG
			    ("Sending LOGON_DEVICE message to remote host %s:%d",
			     control->hostip, control->port);
			DaemonToController out = DAEMON_TO_CONTROLLER__INIT;
			out.code = DAEMON_TO_CONTROLLER__CODE__LOGON_DEVICE;
			if (cmld_get_device_uuid()) {
				DEBUG("Setting uuid: %s",
				      cmld_get_device_uuid());
				out.device_uuid =
				    mem_strdup(cmld_get_device_uuid());
			}
			if (hardware_get_name()) {
				DEBUG("Setting hardware name: %s",
				      hardware_get_name());
				out.logon_hardware_name =
				    mem_strdup(hardware_get_name());
			}
			...
			if (phone_number) {
				...
				out.logon_phone_number =
				    mem_strdup(phone_number);
			}
			if (protobuf_send_message
			    (fd, (ProtobufCMessage *) & out) < 0) {
		...
		ControllerToDaemon *msg =
		    (ControllerToDaemon *) protobuf_recv_message(fd,
								 &controller_to_daemon__descriptor);
		if (msg != NULL) {
			control_handle_message(msg, fd, control);
 ...
 */
static void
control_cb_recv_message_local(int fd, unsigned events, event_io_t * io,
			      void *data)
{
	control_t *control = data;
...
control_cb_accept(int fd, unsigned events, event_io_t * io, void *data)
{
	...
	event_io_t *event =
	    event_io_new(cfd, EVENT_IO_READ, control_cb_recv_message_local,
			 control);
...
control_remote_reconnect_cb(UNUSED event_timer_t * timer, void *data)
{
	...
	int res = sock_inet_connect(control->sock_client, control->hostip,
				    control->port);
	if (-1 == res) {
		DEBUG_ERRNO("Connecting failed to remote host %s:%d",
			    control->hostip, control->port);
		return;
	}
	...
	event_io_t *event =
	    event_io_new(control->sock_client, EVENT_IO_READ | EVENT_IO_WRITE,
			 control_cb_recv_message, control);
	event_add_io(event);
...
static int control_remote_reconnect(control_t * control)
{
	...
	control->reconnect_timer =
	    event_timer_new(CONTROL_REMOTE_RECONNECT_INTERVAL, -1,
...
control_t *control_new(int sock)
{
	...
	event_io_t *event =
	    event_io_new(sock, EVENT_IO_READ, control_cb_accept, control);
	event_add_io(event);
...
int control_remote_connect(control_t * control)
{
	...
	if (!control->connected && !control->reconnect_timer)
		...
		DEBUG
		    ("Tried to connect remote control socket which already attempts a connection");
		return -1;
	}
}
